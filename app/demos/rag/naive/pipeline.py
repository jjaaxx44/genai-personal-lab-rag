...
def ingest(pdf_bytes: bytes, doc_id: str) -> IngestStats:
    ...
    chunks = _chunk_pages(parsed.pages, settings.naive_chunk_size, settings.naive_chunk_overlap)
    ...
    if chunks:
        ...
        ensure_indexes(collection, vector_dims=len(vectors[0]))

    return IngestStats(doc_id=doc_id, chunks=len(chunks), latency_ms=(time.monotonic() - start) * 1000)


@observe(name="naive_ask")
def ask(question: str, doc_id: str, *, top_k: int = 5, **_: object) -> RagResult:
    start = time.monotonic()
    steps: list[str] = []
    collection = get_collection(COLLECTION_NAME)

    query_vector = embed([question])[0]
    steps.append("Embedded the question with bge-small-en-v1.5.")

    results = vector_search(collection, query_vector, doc_id=doc_id, top_k=top_k)
    steps.append(f"$vectorSearch on rag_naive returned {len(results)} passage(s).")

    context = "\n\n".join(f"[{i + 1}] (p.{r['page']}) {r['text']}" for i, r in enumerate(results))
    prompt = PROMPT_TEMPLATE.format(context=context, question=question)

    answer = complete(prompt)
    steps.append("Answered from the numbered context via core.llm.complete().")
